...
export default function ProjectCard({ title, description, link, linkText }: ProjectCardProps) {
  return (
    <motion.div
      className="bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark shadow-card-light dark:shadow-card-dark p-6 rounded-xl h-full group relative overflow-hidden"
      whileHover={{ 
        scale: 1.02,
    ...
    >
      <motion.div
        className="absolute inset-0 bg-gradient-to-r from-primary/20 to-secondary/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
        initial={false}
        animate={{}}
      />
